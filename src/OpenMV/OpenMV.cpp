    ...
    if(usart2::available() > 7)
    {
      ...
      if(camera_data == 255)
      {
        ...
        if(data[6] == crc8(data, 6))
        {
          ...
          if(data[5] != 0)
          {
            ball_new_loc_angle = lead_to_degree_borders(data[4] * 3 - 180);
            ball_new_distance = data[5] * 3;
...
bool camera::check_ball_seen(uint32_t _dT)
{
  return uint32_t(time_service::getCurTime() - _ball_timer) < _dT;
  //return 0;
}
